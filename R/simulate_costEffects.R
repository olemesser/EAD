simulate_costEffects<-function(){
  ...
  DOE<-expand_grid(N_FR = list(c(7)), # number of functional requirements
                   ...
                   DMD_cv = list(c(0.7,2)), # coefficient of variation for demand distribution
                   ...
                   DSM_param=list(c(0,0.14,0.1,2)), # first two entries reffer to the density of the DSMs and the second pair to the cv if the DSM_method='modular' is used.
                   DSM_method='modular',
                   TC = 10^6, # total costs
  ...
  lapply(1:NROW(DOE),function(x){
    ...
    # EAD[[1]]$P$RD[EAD[[1]]$P$RD>0]<-ifelse(sample(0:1,sum(EAD[[1]]$P$RD>0),replace = T)==0,
    #                                        0,
    #                                        EAD[[1]]$P$RD[EAD[[1]]$P$RD>0])
