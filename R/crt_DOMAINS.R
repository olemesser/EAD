...
#' @param N_DD The number of elements in the physical domain (PD)
...
#' @param DMM_method A method how to generate the DSM pattern. The following options are available:
#' \describe{
#'   \item{SDC}{The DMM is generated by treating the \code{DMM_PAR} input as a desired normalized system design complexity.
#'   For further information about this measure see \link[EAD]{measure_designComplexity}.}
#'   \item{DNS}{The DMM is generated by treating the \code{DMM_PAR} input as a desired density.}
#' }
...
crt_DOMAINS<-function(P_FD,
  ...
  #### 2.10. Create Resource consumption of fixed costs ####
  # if(length(RES_COR)==1) RES_COR <- rep(RES_COR,2)
  #   P_RD_fix <- apply(P$RD,2,function(i){
  #     RES_COR<-runif(1,min = RES_COR[1],RES_COR[2])
  #     sqrtConstant1<-sqrt(1-RES_COR^2)
  #     vec<-(RES_COR*i)+(sqrtConstant1*rnorm(length(i)))
  #     vec <- abs(ceiling(vec))
  #     return(vec)
  #   })
  #   P_RD_fix[P$RD==0]<-0
